...
> AngularJS directives.js for Google Maps
...
- **ui-gmap** namespace which will be appended to all directives.js, services, and factories.
...
Main Changes:
- Plural directives.js can evaluate expressions (new) and (old) string properties of models.
...
- **watches**:All directives.js to be moved to using shallow watches (no equality) and or $watchCollection.

- **drop singular layer directives.js**: Singular directives.js while being 'angular', are pretty useless for production and performance unless your size is small. To bring focus to the API our goal for layers (shapes, markers, windows) is to make all of the directives.js plural.

- **remove many watches**, from pural directives.js. We may be de-angularizing somethings to aide in speed. This main change will have to do with watches on models itself. Watchers are a pain, not only are they a performance hit, but it also makes the internals of the api less explicit (where watchers guess and figure out what is going on). We are considering using the control to allow models to be explicitly updated, destroyed, and created. See [Polygons](https://github.com/angular-ui/angular-google-maps/blob/master/src/coffee/directives.js/api/polygons.coffee#L24-L29).
...
__________________
## Getting started
This is a set of directives.js and services for AngularJS `~1.0.7+, ^1.2.2+`.

If you plan to hack on the directives.js or want to run the example, first thing to do is to install NPM dependencies:
...
### Documentation
The various directives.js are documented at [official site](http://angular-google-maps.org).
