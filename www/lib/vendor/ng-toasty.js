...
angular.module('toasty', ['ngAnimate'])
    .service('toasty', ['$rootScope',
        function($rootScope) {
            ...
            this.pop.doyoga = function(options) {
                options.type = 'doyoga';
                that.pop(options);
            }
            this.pop.favorites = function(options) {
                options.type = 'favorites';
                that.pop(options);
            }
            this.pop.wallet = function(options) {
                options.type = 'wallet';
                that.pop(options);
            }
            this.clear = function() {
    ...
    .constant('toastyConfig', {
        ...
        'pushTo': 'bottom', // postion where new toaties are pushed to, top or bottom.
        ...
        'icon-classes': {
            ...
            doyoga: 'toasty-doyoga',
            favorites: 'toasty-favorites',
            wallet: 'toasty-wallet'
        },
                ...
                controller: ['$scope', '$element', '$attrs',
                    function($scope, $element, $attrs) {
                        ...
                        $scope.closeClick = function(toasty) {
                            console.log("Toast ID: ", toasty.id);
                            console.log("Toast Type: ",toasty.type);
                            $scope.removeToasty(toasty.id);
                            var toastyType = toasty.type.slice(7);
                            switch(toastyType) {
                                case 'doyoga':
                                    console.log("doyoga case");
                                    yogijii_store.toast.doyoga = true;
                                    break;
                                case 'favorites':
                                console.log("favorites case");
                                    yogijii_store.toast.favorites = true;
                                    break;
                                case 'wallet':
                                console.log("wallet case");
                                    yogijii_store.toast.wallet = true;
                                    break;
                            //     default:
                            //         default console.log("This is awkward");
                            }
                            console.log(yogijii_store.toast);
                            intel.xdk.cache.setCookie('toast', JSON.stringify(yogijii_store.toast), -1);
                        }
                        // clickToClose
    ...
    .controller('toasty-controller', [ '$rootScope', '$scope', 'toasty', '$window', function($rootScope, $scope, toasty, $window) {
        ...
        // console.log("ATTACHING EVENT LISTENER");
        $rootScope.$on('toasty-routeChange', function() {
            // console.log("CLEARING TOASTY");
            toasty.clear();
        });
    }]);
